...
//import com.example.gh_m.androidweathermain.CityRecyclerViewAdapter;
...
public class FoodDbHelper extends SQLiteOpenHelper {
    ...
    // devicefileexpelorer data data esme proje database sho copy va dar assets gharar midim
    private Context context;
    private static final int DBVERSION = 3;
    // agar ba esme sabet data base table jadid bekhad ezafe she hatman version bayad taghir kone
    //    private static final String DBNAME = "db_food";// age esme tablo avaz kardim eror dad behtare
    //    version data base ro ham avaz konim badesh  dad mishe esm db ham avaz kard
    private static final String DBNAME = "db_food";
    //    public static final String TABLE_FOOD = "tb_food";
    //        in dakhelesh database assets hastesh
    public static final String TABLE_FOOD = "tb_food";

    public static final String[] all_columns = {"id", "name", "description", "healthy", "delicios"
            , "veg", "imgurl"};

    private static final String CMD_CREATE_TB = "CREATE TABLE IF NOT EXISTS '" + TABLE_FOOD + "' (" +
...
//                    1111003 Turn_Up_That_Umami_Kick  Include_asparagus_tomatoes_seaweed_soy_tofu_corn_and_onions_in_your_diet_as_they_are_good_substitute_for_umami  60 60 nothing
                    ...
                    while ((line = reader.readLine()) != null) {//ta vaghti ke reader har khato mikhone
                        // va on khat mohtava dare
                        String[] data = line.split("\t");//\t yani har faselei
                        if (data.length < datasize + 1) {
                            continue;
                            //* continue dar shart be manaye ine ke bere khate bad v split \t
                            // yani harja fasele v rab bod joda kon
                        }
                        long insertId = db.insert(TABLE_FOOD, null,
                                Food.createContentValues(
                                        Long.valueOf(data[0]), // behtare id ham nadad age az ghabl id haye
                                        // khasi dar nazar gerefte nashode bashe  albate bayad tabe bedon
                                        // id ro ham nevesht
                                        data[1],
                                        data[2],
                                        Integer.parseInt(data[3]),
                                        Integer.parseInt(data[4]),
                                        false,// pishfarz hamaro gheir veg midim bara hamin to
                                        // txt filemon nabod
                                        data[6]
                                )
                        );
...
//        Long insertId = db.insert(TABLE_FOOD, null, food.getContentValuesForDb());
    ...
    public void deletFoodToDb(String where) {
        ...
        Log.i("dbhelper", "city deleted with id : " + delete);
        db.close();
    }
...
// inke ye metode add bezarim va begim age in obcet ha ba in moshakhasat to db vojod nadasht addesh kon
}
